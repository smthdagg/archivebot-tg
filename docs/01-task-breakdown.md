...
**最后更新：2026-09-11（公众号订阅跟踪撤销回滚，详见 docs/02 ADR-12）**
...
| M2 归档核心管道 | ✅ 完成并 E2E 验证 | 文本类平台（web/微信/Reddit/X/小红书/微博/知乎）；**视频平台已接入（Phase 2 t_1fb0ee77）：youtube/bilibili/douyin/kuaishou/instagram → fetcher.fetch_video 产出 VideoResult(video_path) → runner.run_video → jobs send_video 交付（复用 50MB 预检），TikTok 尚无 ArchiveBOT service 未适配**；**「图片」输出已改为长截图**（screenshot.py 复用 PDF 模板 full_page PNG，2x 缩放，>40MB 自动转 JPEG；产物命名 标题_YYYY-MM-DD_HHMM.ext，naming.py；Markdown/PDF/截图/ZIP 同名不同扩展；封面回退首图；image_map 数量不匹配时部分映射不再整组丢弃）；**财新付费墙修复（fa06b1c）：cookie 渲染全文进 MD/摘要（vendor 原用匿名 HTML 生成 txt/md，只能出试读版）、黄金图找回（Readability 剥离正文外媒体块，解析后按锚点合并回 content）、p.aitt 提示注入蜜罐剔除、#page2/p0 URL 归一化**；**交付 MD 图片引用改回远程 URL（a63b880，单文件交付无本地 images/ 目录）**；**微信公众号交付修复（15ff08e）：微信 service 不产 content.html 导致仅长截图模式渲染空白页（93KB 标题图），改用过滤后 content.md 渲染；本地图片名→远程URL 映射经 wechat_patch 暂存页面 HTML + data-src 顺序对齐建立，交付 MD 十图全改远程引用**；**财新 /m/ 手机版链接归一化为桌面版（6e27c1d，App 分享链接无黄金图容器且正文为 H5 变体）**；**财新长文分页抓取（ac676ac）：检测 li#purl* 分页导航，逐页 cookie 渲染拼接全文与配图（此前长文只抓第 1 页）；封面图跨页去重（9929927）；正文清理：页内裸 dl 重复封面按图注剔除、#conTit 重复标题、#questions_container AI 组件、自引用回链（68536f4）；PDF/长截图版式：信息块（作者/来源/时间/链接，无标题）移至文末、正文重复标题与 vendor meta 行去重（d99afab）**；**知乎评论区归档（4a22fad → 1f3cd0f，VPS 真实任务 105 验证交付）：zhihu_comments.py 用 Playwright 打开原页面、点击「N 条评论」入口并拦截浏览器自带 x-zse-96 签名的 comment_v5 响应（curl_cffi 免签名只能拿 totals），评论（作者/时间/赞/正文/内嵌子评论）并入渲染源 → PDF/长截图文末「评论区」区块，Markdown 交付追加「## 评论区」；评论为增强特性，无 cookie/失败一律降级跳过不阻塞正文（177 测试全绿）**；**知乎纯问题页归档（815d838 → a3f6c91，VPS 真实任务 108/109 验证交付）：vendor 不支持 `zhuanlan.question/{qid}` 纯问题链接（此前报 Unknown error），zhihu_question.py 用 Playwright 打开问题页、滚动展开正文，提取前 8 个高赞回答（作者/赞同/正文/图片本地化）拼接为合集；点击最高赞回答评论按钮并拦截其 root_comment（评论按钮需轮询等待懒加载）；md 转换偶发崩溃降级纯文本版（正文残缺时）**；**cleaner blocklist 遍历中 decompose 崩溃修复（a3f6c91）：bs4 decompose 会把后代 attrs 置 None，find_all 快照继续遍历 get(class) 崩 AttributeError，改为先收集后删除；评论区容器去 class（blocklist 'comment' 关键词会删掉评论区渲染源）**；本地+容器内真实抓取 E2E-OK | f5104fc → 9de57a4 · t_1fb0ee77 · a9bdae0 · fa06b1c · a63b880 · 15ff08e · 6e27c1d · ac676ac · 22afbcb · 4a22fad · 1f3cd0f · 815d838 · a3f6c91 |
| M3 任务系统 | ✅ 完成并容器内验证 | rq 队列、状态机、取消、并发限制；rq 消费全链路打通 | f5104fc → 9de57a4 |
| M4 Telegram 用户端 | ✅ 完成 | 全部 handler + i18n + 审批流；**格式文案与历史取件已切长截图**（format.images/action.get_images → Screenshot，FileType.SCREENSHOT 新枚举，历史任务 IMAGES_ZIP 自动回退兼容）；**格式菜单选中即收起、任务终态移除残留取消按钮（5973cb6）**；**真实 token 交付联调已验证（a9bdae0，docker compose 全栈，任务 44 长截图 21.8MB 真实上传成功、file_id 落库）** | f5104fc → a9bdae0 · 5973cb6 |
...
| Phase 2 · 公众号订阅跟踪 | ❌ **已撤销回滚（2026-09-11）** | **微信读书路线**（ADR-12）：`/subscribe <公众号名\|文章URL>` 经 weread-omni（Node 22 桥接 `scripts/weread_bridge.mjs`）搜索 `MP_WXS_<id>` 并订阅；交付方式为**订阅级设置**（默认通知+一键归档按钮，可选 auto+默认格式）；worker daemon 线程每 240min 增量检查（synckey 游标回存于 `wx_mp_accounts`），付费文章（payType=2）不分发；管理员 `/weread_login` 扫码（ASCII 二维码）、`/weread_status` 状态页；`-2012` 登录超时提醒管理员、`-2041` 转人工；DB 迁移 a5d985df1063（wx_mp_accounts/wx_subscriptions/wx_pending_articles；VPS 已 stamp+upgrade head）；33 个新测试（桥接协议/检查分发/handler 流）；部署修复：镜像内含 alembic.ini+migrations（1458251）、deploy tar 排除 AppleDouble（9f09883/41d7525）、桥接未登录归类 -2012（71ff283） | e0e9b06 → 71ff283 |
