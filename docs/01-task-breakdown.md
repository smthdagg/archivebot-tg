...
| M2 归档核心管道 | ✅ 完成并 E2E 验证 | 文本类平台（web/微信/Reddit/X/小红书/微博/知乎）；**视频平台已接入（Phase 2 t_1fb0ee77）：youtube/bilibili/douyin/kuaishou/instagram → fetcher.fetch_video 产出 VideoResult(video_path) → runner.run_video → jobs send_video 交付（复用 50MB 预检），TikTok 尚无 ArchiveBOT service 未适配**；**「图片」输出已改为长截图**（screenshot.py 复用 PDF 模板 full_page PNG，2x 缩放，>40MB 自动转 JPEG；产物命名 标题_YYYY-MM-DD_HHMM.ext，naming.py；Markdown/PDF/截图/ZIP 同名不同扩展；封面回退首图；image_map 数量不匹配时部分映射不再整组丢弃）；**财新付费墙修复（fa06b1c）：cookie 渲染全文进 MD/摘要（vendor 原用匿名 HTML 生成 txt/md，只能出试读版）、黄金图找回（Readability 剥离正文外媒体块，解析后按锚点合并回 content）、p.aitt 提示注入蜜罐剔除、#page2/p0 URL 归一化**；**交付 MD 图片引用改回远程 URL（a63b880，单文件交付无本地 images/ 目录）**；**微信公众号交付修复（15ff08e）：微信 service 不产 content.html 导致仅长截图模式渲染空白页（93KB 标题图），改用过滤后 content.md 渲染；本地图片名→远程URL 映射经 wechat_patch 暂存页面 HTML + data-src 顺序对齐建立，交付 MD 十图全改远程引用**；本地+容器内真实抓取 E2E-OK（148 测试全绿） | f5104fc → 9de57a4 · t_1fb0ee77 · a9bdae0 · fa06b1c · a63b880 · 15ff08e |
