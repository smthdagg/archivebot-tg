...
| Phase 2 · 公众号订阅跟踪 | ✅ 代码完成（待 VPS 联调） | **微信读书路线**（ADR-12）：`/subscribe <公众号名\|文章URL>` 经 weread-omni（Node 22 桥接 `scripts/weread_bridge.mjs`）搜索 `MP_WXS_<id>` 并订阅；交付方式为**订阅级设置**（默认通知+一键归档按钮，可选 auto+默认格式）；worker daemon 线程每 240min 增量检查（synckey 游标回存于 `wx_mp_accounts`），付费文章（payType=2）不分发；管理员 `/weread_login` 扫码（ASCII 二维码）、`/weread_status` 状态页；`-2012` 登录超时提醒管理员、`-2041` 转人工；DB 迁移 a5d985df1063（wx_mp_accounts/wx_subscriptions/wx_pending_articles）；33 个新测试（桥接协议/检查分发/-handler 流） | 本分支 feat/weread-subscribe |
