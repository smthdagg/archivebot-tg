...
# ---------- 可配置项（按需修改） ----------
VPS_HOST="${VPS_HOST:-<VPS-IPv4>}"            # VPS IP（IPv4）
VPS_PORT="${VPS_PORT:-2222}"                      # SSH 端口
VPS_USER="${VPS_USER:-root}"                      # SSH 用户
VPS_DIR="${VPS_DIR:-/opt/archivebot}"             # VPS 部署目录
SSH_KEY="${SSH_KEY:-$HOME/Documents/sshkey.pem}"  # 默认用 VPS 密钥；可用 SSH_KEY 环境变量覆盖
# 需要排除的本地目录/文件（打包时跳过）
EXCLUDES=(.git .gitmodules .gitignore .venv .pytest_cache .ruff_cache .worktrees .serena .zcode
          __pycache__ "*.pyc" .DS_Store storage data)
# -----------------------------------------
